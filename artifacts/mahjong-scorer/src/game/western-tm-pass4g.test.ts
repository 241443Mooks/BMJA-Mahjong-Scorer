...
describe('western-tm@0.1 Pass 4G dragon hybrids and own-wind catalogue wave', () => {
  ...
  it('uses playerWind, not prevailingWind, for Little Brother and Hovering Angel', () => {
    ...
    expect(score({ ...angel('north'), sets: angel('north').sets.map((group) => group.id === 'own' ? { ...group, visibility: 'exposed' as const } : group) }, 'north').specialHands.find(({ id }) => id === 'own-wind-meld-with-dragon-pair-and-three-suit-chows')?.matched).toBe(false);
  ...
  it('finds hybrid and grouped own-wind fishing with their profile exposure values', () => {
    ...
    expect(findFishing({ ...angelFishing, sets: angelFishing.sets.map((group) => group.id === 'b' ? { ...group, visibility: 'exposed' as const } : group) }, 'own-wind-meld-with-dragon-pair-and-three-suit-chows', 'north')?.fishingValue).toBe(400);
  });
});
