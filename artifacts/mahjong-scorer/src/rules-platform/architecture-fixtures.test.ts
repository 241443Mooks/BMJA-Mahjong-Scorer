...
const envFor = (fixture: Fixture): ResolverEnvironment => {
  ...
  const registry = new RegistryBank([...architectureSeedEntries.map(entry => parameterised.has(entry.id) ? { ...entry, parameterSchemaId: parameterised.get(entry.id) } : entry), { id: 'classical.concealed-hand-additive-points', category: 'classical', status: 'architecture-only' }, { id: 'classical.concealed-hand-double', category: 'classical', status: 'architecture-only' }, { id: 'evidence.riichi-declaration', category: 'evidence', status: 'architecture-only' }]);
...
describe('phase 3 cross-profile semantic pressure', () => {
  it('C01 shares Three Great Scholars structure while retaining isolated local bindings and values (row 19)', () => {
    const registry = new RegistryBank([
      { id: 'pattern.three-great-scholars', category: 'pattern', status: 'architecture-only' },
      { id: 'classical.binding.bmja-three-great-scholars', category: 'classical', status: 'architecture-only' },
      { id: 'classical.binding.western-three-great-scholars', category: 'classical', status: 'architecture-only' },
      { id: 'classical.value.bmja-three-great-scholars', category: 'classical', status: 'architecture-only' },
      { id: 'classical.value.western-three-great-scholars', category: 'classical', status: 'architecture-only' },
      { id: 'classical.policy.bmja-three-great-scholars', category: 'classical', status: 'architecture-only' },
      { id: 'classical.policy.western-three-great-scholars', category: 'classical', status: 'architecture-only' },
    ]);
    const canonical = registry.get('pattern', 'pattern.three-great-scholars');
    const bmja = { canonicalPatternId: canonical.id, bindingId: 'classical.binding.bmja-three-great-scholars', valueId: 'classical.value.bmja-three-great-scholars', policyId: 'classical.policy.bmja-three-great-scholars' };
    const western = { canonicalPatternId: canonical.id, bindingId: 'classical.binding.western-three-great-scholars', valueId: 'classical.value.western-three-great-scholars', policyId: 'classical.policy.western-three-great-scholars' };
    expect(canonical.metadata).toBeUndefined();
    expect(bmja.canonicalPatternId).toBe(western.canonicalPatternId);
    expect(bmja.bindingId).not.toBe(western.bindingId);
    expect(bmja.valueId).not.toBe(western.valueId);
    expect(bmja.policyId).not.toBe(western.policyId);
    const changedWestern = { ...western, valueId: 'classical.value.western-three-great-scholars-revised' };
    expect(canonical.id).toBe('pattern.three-great-scholars');
    expect(bmja).toEqual({ canonicalPatternId: 'pattern.three-great-scholars', bindingId: 'classical.binding.bmja-three-great-scholars', valueId: 'classical.value.bmja-three-great-scholars', policyId: 'classical.policy.bmja-three-great-scholars' });
    expect(western.valueId).toBe('classical.value.western-three-great-scholars');
    expect(changedWestern.valueId).not.toBe(western.valueId);
  });

  it('C02 keeps a narrow wait and broader only-possible-winning-tile condition distinct', () => {
    const registry = new RegistryBank([
      { id: 'wait.narrow-single-tile', category: 'wait', status: 'architecture-only' },
      { id: 'condition.only-possible-winning-tile', category: 'condition', status: 'architecture-only' },
    ]);
    const narrowWait = registry.get('wait', 'wait.narrow-single-tile');
    const broaderCondition = registry.get('condition', 'condition.only-possible-winning-tile');
    expect(narrowWait.id).not.toBe(broaderCondition.id);
    expect(narrowWait.category).toBe('wait');
    expect(broaderCondition.category).toBe('condition');
  });

  it('C03 keeps Riichi-only declaration evidence out of the Classical fixture', () => {
    const riichi = inspectProfile(fixtures.find(fixture => fixture.id === 'A05')!.definition, envFor(fixtures.find(fixture => fixture.id === 'A05')!));
