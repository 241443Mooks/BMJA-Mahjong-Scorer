...
export type ResolvedScorerExample = { id: string; name: string; hand: MahjongHand; context: HandScorerContext; returnHref: string; };
export const resolveScorerExample = (id: string | null | undefined): ResolvedScorerExample | undefined => {
  ...
  if (worked) return { id: worked.id, name: worked.title, hand: worked.hand, context: scoringExampleContext(worked), returnHref: worked.returnHref };
  const special = specialHandExampleById(id);
  if (special) {
    return { id: special.id, name: special.name, hand: special.hand, context: exampleHandScorerContext(special), returnHref: `/special-hands#${special.id}` };
  }
  return undefined;
};
