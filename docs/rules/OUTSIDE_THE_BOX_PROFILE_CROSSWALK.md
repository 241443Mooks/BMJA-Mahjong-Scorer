...
| 88E | implemented: explicit `RoundIncident` evidence (`incorrect-hand`, `false-discard-name`, `false-mah-jong`, `wrong-tile-claim`, `cannon`), profile preparation, settlement wrapper, replay/recovery and ledger evidence | automatic cannon/no-choice proof remains intentionally out of scope; the guide records a 50 false-name claim payment but does not establish its recipient, so that single form is rejected rather than inventing a destination |
...
## 10. 88E executable incident policy

Only `outside-the-box@0.1` accepts structured round incidents. BMJA and provisional T&M reject non-empty incident input. The profile prepares the submitted round before both preview and confirmation: too-few and late-wrong-claim players cannot win; too-many players cannot win, have their effective score canonicalised to zero, and their submitted detailed/manual score evidence is discarded. A corrected wrong claim has no scoring effect.

False Mah Jong is an additive transfer only when a hand was exposed: the declarer pays every other player half the active OTB limit, with `eastMultiplier: 1`. A false-name Mah Jong or active Cannon replaces ordinary settlement: calculate ordinary winner payments first, sum their actual amounts (including East effects), then record exactly one liability transfer to the winner. `No choice! accepted` retains Cannon evidence but cancels that override, restoring ordinary settlement. Competing active liability overrides are rejected. Incidents are manual table evidence; this pass does not infer claims, Cannon, danger, fishing, alternatives, or exposure.
